...
mkdir -p "$fixture_home" "$codex_home" "$workspace" \
...
run_isolated() {
  env -i \
    HOME="$fixture_home" CODEX_HOME="$codex_home" \
...
[[ "$(<"$codex_home/config.toml")" == "$base_config_before" ]] \
  || fail "parallel-work profile checks changed the base fixture config"

print -- "PASS: parallel-work profile inherits defaults, disables the Engram plugin, and retains MCP availability"
